        ...
        <div className={clsx(
            ...
            <div className="p-4 bg-white/5 border-b border-white/5 flex justify-between items-center z-10">
                <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-white font-bold text-[10px] tracking-[0.2em] uppercase">Threat_Engine</h3>
                </div>
                <button
                    onClick={onToggle}
                    className={clsx(
            ...
            <div className="p-4 flex-1 flex flex-col z-10 relative">
                <AnimatePresence mode="wait">
                    {showAnomalies ? (
                            ...
                            {metrics ? (
                                isCritical ? (
                                    <div className="p-3 bg-red-900/10 border border-red-500/30 rounded flex items-start gap-3">
                                        ...
                                        <div>
                                            <h4 className="text-red-400 font-bold text-[11px] uppercase tracking-wider">Breach Warning</h4>
                                            <p className="text-red-300/50 text-[9px] mt-1 leading-relaxed">
                                                INTEGRITY RISK DETECTED IN {metrics.name.toUpperCase()} DOMAIN.
                                                NON-COGNITIVE ATTENDANCE &gt; 15%.
                                            </p>
                                            <div className="mt-2 text-[10px] font-black text-red-500 flex items-center gap-1">
                                                <Zap size={10} /> {metrics.anomalies} ACTIVE THREATS
                                            </div>
                                        </div>
                                    </div>
                                ) : (
                                    <div className="p-3 bg-blue-900/10 border border-blue-500/30 rounded flex items-center gap-3">
                                        ...
                                        <span className="text-blue-400 text-[10px] font-bold uppercase tracking-widest">Integrity Secured</span>
                                    </div>
                                )
                            ) : (
                                <div className="text-gray-500 text-[9px] italic uppercase tracking-widest">Select regional node to scan...</div>
                            )}
                        </motion.div>
                    ) : (
                        <motion.div
                        ...
                        >
                            <ShieldAlert size={32} />
                            <span className="text-[8px] font-black tracking-[0.3em]">ENGINE_OFFLINE</span>
                        </motion.div>
                    )}
                </AnimatePresence>
            </div>
