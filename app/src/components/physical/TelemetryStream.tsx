...
import { OdometerCounter } from '../shared/OdometerCounter';
...
export const NeuralLinkTelemetry: React.FC = () => {
    ...
    return (
        <div className="grid grid-rows-[44px_1fr_480px_28px] h-full w-full bg-[#030303] text-white font-['JetBrains_Mono',monospace] border border-blue-500/20 relative overflow-hidden shadow-2xl">
            ...
            <div className="flex items-center justify-between px-6 border-b border-blue-500/20 bg-black/40 backdrop-blur-md z-20 overflow-hidden shrink-0">
                <div className="flex items-center gap-6">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-[11px] font-black uppercase tracking-[0.3em]">Neural Link Telemetry</span>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-[220px_minmax(0,1fr)_380px] min-h-0 overflow-hidden border-b border-blue-500/20 z-10">
                {/* 2.1 LEFT STATS */}
                <div className="border-r border-blue-500/10 flex flex-col p-4 bg-black/20 gap-4 overflow-y-auto no-scrollbar min-h-0 shrink-0">
                    <div className="space-y-4">
                        <div className="p-3 bg-blue-500/5 border border-blue-500/20 rounded-sm shadow-[inset_0_0_10px_rgba(59,130,246,0.1)]">
                            <span className="text-[8px] font-bold text-blue-400 uppercase">Input Saturation</span>
                        ...
                        <div className="grid grid-cols-2 gap-2 text-center text-[8px] font-bold">
                            <div className="p-2 bg-white/5 border border-white/5 uppercase">
                                <div className="text-white/40 mb-1 font-black">Sync</div>
                                <div className="text-blue-400">99.8%</div>
                            </div>
                            <div className="p-2 bg-white/5 border border-white/5 uppercase">
                                <div className="text-white/40 mb-1 font-black">Loss</div>
                                <div>0.0%</div>
                            </div>
                        </div>
                ...
                <div className="relative overflow-hidden flex flex-col group min-h-0 min-w-0">
                    ...
                    <div className="grid grid-cols-[70px_120px_80px_100px_90px_1fr] gap-2 px-4 h-8 items-center border-b border-white/10 bg-white/5 text-[8px] font-black uppercase tracking-widest text-white/40 shrink-0">
                        <div>Time</div><div>Subject</div><div>Cluster</div><div>Topic</div><div>Action</div><div>Context</div>
                    </div>
                            ...
                            {recentLogs.map((log, i) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div><span className="px-1.5 py-0.5 text-[8px] font-black uppercase rounded-[1px] text-black" style={{ backgroundColor: CLUSTER_COLORS[log.cluster] }}>{log.cluster}</span></div>
                                        <div className="text-white/60 truncate uppercase text-[9px]">{log.topic}</div>
                                        <div className={clsx("text-[9px] font-black uppercase tracking-tighter px-2 py-0.5 rounded-sm w-fit", log.action === 'Purchase' ? "text-emerald-400 bg-emerald-500/10" : log.action === 'Test Pass' ? "text-blue-400 bg-blue-500/10" : "text-white/30")}>{log.action}</div>
                        ...
                        <div className="space-y-6">
                            <div className="bg-white/5 p-4 rounded-sm border border-white/10">
                                <div className="flex justify-between items-start mb-4">
                                    <div><h4 className="text-[12px] font-black text-white">{selectedRow.userId}</h4><span className="text-[9px] text-white/30 uppercase tracking-widest">{selectedRow.cluster} DOMAIN</span></div>
                                    <div className="text-right"><div className="text-[14px] font-black text-blue-400">{(selectedRow.cognitiveLoad || 0).toFixed(1)}</div><div className="text-[7px] text-white/40 uppercase">Load Index</div></div>
                                </div>
            ...
            <div className="relative bg-black border-t border-blue-500/20 overflow-hidden min-h-0">
                <div className="absolute top-4 left-6 z-10 flex items-center gap-2 pointer-events-none">
                    ...
                    <span className="text-[9px] font-black uppercase tracking-widest text-white/30">Dependent Holographic Engine</span>
                </div>
                <div className="absolute inset-0 w-full h-full overflow-hidden">
                    <Suspense fallback={<div className="flex items-center justify-center h-full text-white/20 text-[10px]">INITIALIZING WEBGL...</div>}>
