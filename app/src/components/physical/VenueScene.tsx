...
const VenueModel = ({ activeCluster }: { activeCluster: string | null }) => {
    // Refined Palette (Synced with TelemetryStream)
    const clusters: { name: ClusterType, color: string, angle: number }[] = [
        { name: 'Science', color: '#3B82F6', angle: 0 },
        { name: 'Technology', color: '#22D3EE', angle: 60 },
        { name: 'Economics', color: '#FBBF24', angle: 120 },
        { name: 'Society', color: '#10B981', angle: 180 },
        { name: 'Politics', color: '#94A3B8', angle: 240 },
        { name: 'Art', color: '#6366F1', angle: 300 },
    ];
