...
export const CharacterSelector: React.FC<CharacterSelectorProps> = ({ themeColor }) => {
    ...
    return (
        <div className="flex flex-col gap-3">
            ...
            {/* Agent List */}
            <div className="flex gap-2 relative z-10 w-full overflow-x-auto no-scrollbar">
                {selectableUsers.map((user) => {
                    ...
                    return (
                        <button
                            ...
                            className={`
                                 flex items-center gap-3 p-2 pr-4 rounded-sm transition-all duration-300 relative group/agent flex-1 min-w-[120px] border
                        ...
                        >
                            {/* Avatar Placeholder */}
                            <div className="relative w-8 h-8 rounded-full overflow-hidden bg-black border border-white/10 group-hover/agent:border-white/30 transition-all flex-none">
                                <div className="absolute inset-0 flex items-center justify-center text-[10px] font-bold text-gray-500 group-hover/agent:text-white transition-colors">
                                    {user.name.charAt(0)}
                                </div>
                            </div>
