    ...
    return (
        <div className="w-full h-full relative" onClick={() => setActiveZone(null)}>
            <Canvas>
                ...
                {clusterMetrics.map((metric) => (
                    <ClusterSphere
                        ...
                        label={CLUSTER_TRANSLATIONS[metric.name] || metric.name}
            ...
            <div className="absolute top-4 left-4 pointer-events-none">
                <h3 className="text-white font-bold opacity-50">РЕГИОНАЛЬНЫЙ КОНТРОЛЬ // {activeZone ? (CLUSTER_TRANSLATIONS[activeZone] || activeZone).toUpperCase() : 'ВСЕ ЗОНЫ'}</h3>
            </div>
            <div className="absolute bottom-4 left-4 pointer-events-none">
                <p className="text-[10px] text-gray-500">НАЖМИТЕ НА СФЕРУ ДЛЯ ФОКУСИРОВКИ</p>
            </div>
        </div>
    );
};
