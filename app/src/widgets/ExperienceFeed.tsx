...
const getSemanticColor = (content: string) => {
    ...
    if (upper.includes('ОБЩЕСТВО') || upper.includes('SOCIETY')) return '#10b981'; // Emerald
    if (upper.includes('ТЕХНОЛОГИИ') || upper.includes('TECHNOLOGY')) return '#3b82f6'; // Blue
    if (upper.includes('ЭКОНОМИКА') || upper.includes('ECONOMICS')) return '#f59e0b'; // Amber
    if (upper.includes('ПОЛИТИКА') || upper.includes('POLITICS')) return '#ef4444'; // Red
    if (upper.includes('ИСКУССТВО') || upper.includes('ART')) return '#d946ef'; // Fuchsia
    if (upper.includes('НАУКА') || upper.includes('SCIENCE')) return '#06b6d4'; // Cyan
